...
var (
	...
	createBookStructType = graphql.NewObject(graphql.ObjectConfig{
		Name: "CreateBookStruct",
		Fields: graphql.Fields{
			"isbn": &graphql.Field{
				Type: scalar.String,
			},
...
func (r *bookServiceResolver) FieldCreateBook() *graphql.Field {
	field := &graphql.Field{
		Name:        "/book.BookService/CreateBook",
		Description: "",
...
func (r *bookServiceResolver) resolveCreateBook(p graphql.ResolveParams) (interface{}, error) {
	in := &CreateBookStruct{}
	valueIsbn, ok := p.Args["isbn"].(string)
	if !ok {
		valueIsbn = ""
	...
	in.Isbn = valueIsbn
	valueTitle, ok := p.Args["title"].(string)
	if !ok {
		valueTitle = ""
	}
	in.Title = valueTitle
...
func RegisterBookServiceHandlerClient(mux *runtime.ServeMux, client BookServiceClient) error {
	...
	// gRPC /book.BookService/CreateBook
